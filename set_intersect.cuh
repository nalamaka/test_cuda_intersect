...
__forceinline__ __device__ T intersect_bs(T* a, T size_a, T* b, T size_b) {
...
__forceinline__ __device__ T intersect_bs_cache(T* a, T size_a, T* b, T size_b) {
  //if (size_a == 0 || size_b == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE-1); // thread index within the warp
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the CTA
  __shared__ T count[WARPS_PER_BLOCK];
...
__forceinline__ __device__ T intersect(T* a, T size_a, T *b, T size_b){
  ...
  //else
    ...
    return intersect_bs_cache(a, size_a, b, size_b);
}

template <typename T = vidType>
// __forceinline__ __device__ T intersect_bs(T* a, T size_a, T* b, T size_b, T upper_bound, T* c) {
__forceinline__ __device__ T intersect_bs(T* a, T size_a, T* b, T size_b, T upper_bound) {
  if (size_a == 0 || size_b == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE-1); // thread index within the warp
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the CTA
  __shared__ T count[WARPS_PER_BLOCK];
...
__forceinline__ __device__ T intersect_bs_cache(T* a, T size_a, T* b, T size_b, T upper_bound) {
  if (size_a == 0 || size_b == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE-1); // thread index within the warp
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the CTA
  __shared__ T count[WARPS_PER_BLOCK];
...
__forceinline__ __device__ T intersect(T* a, T size_a, T *b, T size_b, T upper_bound) {
  // return intersect_bs_cache(a, size_a, b, size_b, upper_bound, c);
  return intersect_bs_cache(a, size_a, b, size_b, upper_bound);
}
